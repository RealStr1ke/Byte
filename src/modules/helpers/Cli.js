...
class Cli {
	...
	async invite() {
		...
		try {
			const fetching = await this.client.guilds.fetch(input.id);
			const guild = this.client.guilds.cache.get(fetching.id);
			const channel = guild.channels.cache.filter(c => c.type === 'text' && c.permissionsFor(guild.me).has('CREATE_INSTANT_INVITE')).first();
